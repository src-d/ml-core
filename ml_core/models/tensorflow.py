...
class TensorFlowModel(Model):
    ...
    def construct(
        self,
        graphdef: "tensorflow.GraphDef" = None,  # noqa: F821
        ...
        if graphdef is None:
            ...
            for node in graphdef.node:
                node.device = ""
                graphdef = graph_util.convert_variables_to_constants(session, graphdef, outputs)
    ...
    def _generate_tree(self) -> dict:
        return {
            "graphdef": numpy.frombuffer(self._graphdef.SerializeToString(), dtype=numpy.uint8)
        }
