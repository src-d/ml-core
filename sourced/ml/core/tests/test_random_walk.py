...
from sourced.ml.core.algorithms.uast.ids_to_bag import FakeVocabulary
from sourced.ml.core.algorithms.uast.struct_to_bag import Uast2RandomWalks
...
class RandomWalkTests(unittest.TestCase):
    def setUp(self):
        ...
        self.uast2walk = Uast2RandomWalks(
            p_explore_neighborhood=0.5,
    ...
    def test_rw(self):
        for walk in self.uast2walk(self.uast):
            for i in range(len(walk) - 1):
                self.assertNotEqual(
                    walk[i], walk[i + 1], "Two neighbours nodes should not be the same"
                )
