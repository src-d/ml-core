...
flags.DEFINE_string(
    "input_base_path", None, "Directory containing input shards, vocabularies, " "and marginals."
)
flags.DEFINE_string("output_base_path", None, "Path where to write the trained embeddings.")
...
flags.DEFINE_integer(
    "submatrix_rows",
    4096,
    "Rows in each training submatrix. This must match " "the training data.",
)
flags.DEFINE_integer(
    "submatrix_cols",
    4096,
...
flags.DEFINE_float("loss_multiplier", 1.0 / 4096, "constant multiplier on loss.")
flags.DEFINE_float("confidence_exponent", 0.5, "Exponent for l2 confidence function")
...
flags.DEFINE_integer("num_concurrent_steps", 2, "Number of threads to train with")
flags.DEFINE_integer("num_readers", 4, "Number of threads to read the input data and feed it")
flags.DEFINE_float("num_epochs", 40, "Number epochs to train for")
flags.DEFINE_float(
    "per_process_gpu_memory_fraction", 0, "Fraction of GPU memory to use, 0 means allow_growth"
)
flags.DEFINE_integer("num_gpus", 0, "Number of GPUs to use, 0 means all available")
flags.DEFINE_string("logs", "", "Path for TensorBoard logs (empty value disables them)")
...
def embeddings_with_init(vocab_size, embedding_dim, name):
    ...
    return tf.get_variable(
        ...
        initializer=tf.random_normal_initializer(stddev=math.sqrt(1.0 / embedding_dim)),
    )
...
def count_matrix_input(filenames, submatrix_rows, submatrix_cols):
    ...
    sparse_indices = tf.concat(
        axis=1, values=[tf.expand_dims(sparse_local_row, 1), tf.expand_dims(sparse_local_col, 1)]
    )
    count = tf.sparse_to_dense(
        sparse_indices, [submatrix_rows, submatrix_cols], sparse_count, validate_indices=False
    )

    queued_global_row, queued_global_col, queued_count = tf.train.batch(
        [global_row, global_col, count], batch_size=1, num_threads=FLAGS.num_readers, capacity=32
    )
...
def write_embeddings_to_disk(config, model, sess):
    ...
    write_embedding_tensor_to_disk(
        row_vocab_path, row_embedding_output_path, sess, model.row_embedding
    )
    ...
    write_embedding_tensor_to_disk(
        col_vocab_path, col_embedding_output_path, sess, model.col_embedding
    )
...
class SwivelModel:
    ...
    def __init__(self, config):
        ...
        with tf.device("/cpu:0"):
            ...
            self.row_embedding = embeddings_with_init(
                embedding_dim=config.embedding_size, vocab_size=self.n_rows, name="row_embedding"
            )
            self.col_embedding = embeddings_with_init(
                embedding_dim=config.embedding_size, vocab_size=self.n_cols, name="col_embedding"
            )
        ...
        devices = (
            ["/gpu:%d" % i for i in range(FLAGS.num_gpus)]
            if FLAGS.num_gpus > 0
            else get_available_gpus()
        )
        ...
        for dev in devices:
            with tf.device(dev):
                with tf.name_scope(dev[1:].replace(":", "_")):
                    # ===== CREATE GRAPH =====
                    # Fetch embeddings.
                    selected_row_embedding = tf.nn.embedding_lookup(self.row_embedding, global_row)
                    selected_col_embedding = tf.nn.embedding_lookup(self.col_embedding, global_col)
        ...
        with tf.device("/cpu:0"):
            # ===== MERGE LOSSES =====
            l2_loss = tf.reduce_mean(tf.concat(axis=0, values=l2_losses), 0, name="l2_loss")
            sigmoid_loss = tf.reduce_mean(
                tf.concat(axis=0, values=sigmoid_losses), 0, name="sigmoid_loss"
            )
            ...
            for grads in all_grads:
                apply_gradient_ops.append(opt.apply_gradients(grads, global_step=self.global_step))
    ...
    def initialize_summary(self, sess):
        ...
        self.embedding10k = tf.Variable(
            tf.zeros((length, self._config.embedding_size)), name="top10k_embedding"
        )
    ...
    def write_summary(self, sess):
        ...
        summary, _, global_step = sess.run((self.summary, assignment, self.global_step))
        self.writer.add_summary(summary, global_step)
        self.saver.save(sess, os.path.join(FLAGS.logs, "embeddings10k.checkpoint"), global_step)
...
def main(_):
    ...
    log(
        ...
        + "\n".join(
            "\t{:20} {}".format(key, value)
            for key, value in sorted(FLAGS.__dict__.items())
    ...
    with tf.Graph().as_default():
        ...
        if FLAGS.per_process_gpu_memory_fraction > 0:
            gpu_opts["per_process_gpu_memory_fraction"] = FLAGS.per_process_gpu_memory_fraction
        else:
        ...
        n_steps_per_thread = n_total_steps / (FLAGS.num_concurrent_steps * model.devices_number)
        ...
        msg = (
            "%%%dd/%%d submatrices trained (%%.1f%%%%), " "%%5.1f submatrices/sec | loss %%f"
